...
[extra_path]: Any number of extra_path tags may be included. Each one defines an additional animation to run at the same time.
    ...
    Available keys: x,y,image,linger,transpose,interpolation
...
local function load_path(cfg, container_name)
    ...
    temp = cfg.x or helper.wml_error(container_name..": missing required x= attribute")
    animation.x_locs, animation.num_locs = load_list(temp)
    temp = cfg.y or helper.wml_error(container_name..": missing required y= attribute")
...
function wesnoth.wml_actions.animate_path(cfg)
    ...
    for i = 0, frames do
        for j = 1, num_animations do
            ...
            animation[j].target_hex_x, animation[j].target_hex_y, x, y = calc_image_hex_offset(hex_x,hex_y,x,y)
            animation[j].image_name = get_image_name_with_offset( target_hex_x, target_hex_y, x, y, animation[j].images[i%animation[j].num_images])
